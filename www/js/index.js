...
window.onload = function() {
    ...
    can = draw_area.getContext('2d');//取得2d上下文对象
	...
	$('resetnode').onclick = function() {
		if(confirm('确定重置景点吗？')) {
			...
			can.clearRect(0,0,770,500);//770，500为画布大小
		}		
	}
}
function requestMapInfo(Id){
    ajax({
        url: '/mapInfo',
        data: {act:'searchMap',id:Id},
        type: 'get',
        success: function (str){
            var json = JSON.parse(str);
            if(json.ok){
                addMapInfo(json.result);
            }else{
                alert("获取数据失败！！");
            }
        },
...
//请求拓扑图信息
function requestGrahInfo(mapId){
    ajax({
        url: '/mapInfo',
        data: {act:'searchGrah',id:mapId},
        type: 'get',
        success: function (str){
            var json = JSON.parse(str);
            if(json.ok){
                disPlayGrah(json.result);
            }else{
                alert("获取数据失败！！");
            }
        },
...
function disPlayGrah(GrahInfo) {
    var oDisplay = document.getElementById("dispalyGrah");
    oDisplay.onclick = function(){
        // console.log(paths);
        // console.log(sights);
        // console.log(GrahInfo[0].paths);
        // console.log(GrahInfo[0].sights);
       if(paths.length==0 && sights.length == 0){
           // paths = GrahInfo[0].paths;
           // sights = GrahInfo[0].sights;
           var pathsInfo = GrahInfo[0].paths;
           console.log(pathsInfo);
           console.log(GrahInfo[0].paths);
           // for(var i=0;i<GrahInfo[0].paths.length;i++){
           //     paths[i] = new PathNode();
           //     paths[i].startNode = GrahInfo[0].paths[i].startNode;
           //     paths[i].endNode = GrahInfo[0].paths[i].endNode;
           //     paths[i].pathLength = GrahInfo[0].paths[i].pathLength;
           // }
           // for(var i = 0;i<GrahInfo[0].sights.length;i++){
           //     sights[i] = new SightNode();
           //     sights[i].startNode = GrahInfo[0].sights[i].startNode;
           //     sights[i].endNode = GrahInfo[0].sights[i].endNode;
           //     sights[i].pathLength = GrahInfo[0].sights[i].pathLength;
           // }
           console.log(paths);
           console.log(sights);
       }
...
function addMapInfo(mapInfoList){
    var oSelectMap  = document.getElementById('selectMap');
    var oContGround = document.getElementsByClassName("content_ground")[0];
    for(var  i=0;i<mapInfoList.length;i++){
        var newOption = document.createElement("option");
        newOption.innerText = mapInfoList[i].name;
        oSelectMap.appendChild(newOption);
    }
    oSelectMap.onchange = function(){
        var canvasMap = $('draw_area');
        for(var i=0;i<mapInfoList.length;i++){
            if(oSelectMap.selectedIndex==i){
                can.clearRect(0,0,weight,height);//刷新画布
                canvasMap.style.backgroundImage="url(images/"+mapInfoList[i].mapSrc;
                oContGround.style.backgroundImage = "url(images/"+mapInfoList[i].groundSrc;
                requestGrahInfo(4);
            }
        }
    };
...
function addNode() {
	...
	can.beginPath();
	can.arc(mouse_pos.x, mouse_pos.y, 10, 0, 360, false);
	can.strokeStyle="rgb(255,0,0)";
	can.fillStyle = "rgb(255,0,0)";
	can.fill();
	can.font = "12px Courier New";
...
function redrawNode() {	
	for(var i = 0; i < nodeNum; i++) {
		can.beginPath();
		can.arc(sights[i].XPos, sights[i].YPos, 10, 0, 360, false);
		can.strokeStyle="rgb(255,0,0)";
		can.fillStyle = "rgb(255,0,0)";
		can.fill();
...
function autoDrawLine(){
    for(var i=0;i<paths.length;i++){
        var pathLength = paths[i].pathLength;
        if(pathLength) {
            (function(o){
                can.beginPath();
                can.strokeStyle = "rgb(0, 0, 255)";	//设置画笔颜色
                can.fillStyle = "rgb(0,0,0)";
                can.moveTo(sights[paths[o].startNode-1].XPos, sights[paths[o].startNode-1].YPos);
                can.lineTo(sights[paths[o].endNode-1].XPos, sights[paths[o].endNode-1].YPos);
                var textXPos = (sights[paths[o].startNode - 1].XPos + sights[paths[o].endNode - 1].XPos) / 2;
                var textYPos = (sights[paths[o].startNode - 1].YPos + sights[paths[o].endNode - 1].YPos) / 2;
                can.fillText(pathLength, textXPos, textYPos);
                can.stroke();
                can.closePath();
                setOperationInfo(2,-1 ,-1);
...
function reDrawGrah(){
    can.clearRect(0,0,weight,height);//刷新画布
    var pathInfo = JSON.stringify(paths);
    console.log(pathInfo);
    var sightsInfo = JSON.stringify(sights);
    console.log(sightsInfo);
    redrawNode();
    autoDrawLine();
}
//最短路径查询
function queryPaths() {
    $('minpaths').innerHTML = '';//重置最短路径显示
    reDrawGrah();
	if(nodeNum < 2) {
		alert('请先设置景点和路径');
		return;
